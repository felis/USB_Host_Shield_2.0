...
//**** Wii Camera****//
					...
					/* When it works it will alsy have intensity and bounding box data */
					/*
...
uint8_t WII::statusRequestPublic() { //This is a public version of the functio so it can be called from Arduino. It does not work, it is supposed to return if the IR is enabled. I will try to fix it
    uint8_t cmd_buf[3];
    cmd_buf[0] = 0xA2; // HID BT DATA_request (0xA0) | Report Type (Output 0x02)
    cmd_buf[1] = 0x15;
    cmd_buf[2] = (HIDBuffer[2] & 0x01); // Keep the rumble bit
