...
Wii example. Contact Kristian:  http://blog.tkjelectronics.dk/ kristianl@tkjelectronics.com.
  ...
  if(Wii.wiimoteConnected) {
    if(Wii.getButtonClick(HOME)) { // You can use getButtonPress to see if the button is held down
      ...
      if(Wii.getButtonClick(ONE)) {
        ...
        //Wii.statusRequestPublic(); //This function isn't working right now
      }
      
      if(Wii.getButtonClick(TWO)) //check status request. Returns if IR is intialized or not (Serial Monitor only)
        Wii.statusRequestPublic(); //isn't working proberly. It returns "extension disconnected", will fix soon
